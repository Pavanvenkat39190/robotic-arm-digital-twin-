...
export default function App() {
  ...
  // Socket connection useEffect
   useEffect(() => {
    ...
    function onConnect() {
      ...
      setAlert({ message: "✅ Connected to server" });
    }

    function onDisconnect(reason) {
      console.log('Disconnected from server:', reason);
      setAlert({ message: "⚠️ Disconnected from server. Trying to reconnect..." });
    }
    ...
    function onInitialData(payload) {
      ...
      setAlert({ message: payload.isShutdown ? "⚠️ System is offline" : "✅ All systems normal" });
    }
    ...
    function onSystemReset(payload) {
      console.log("Received system reset:", payload);
      setIsShutdown(false);
      setData(payload.data || []);
      setHealth(payload.health || 100);
      setFaults(payload.faults || {});
      setMaintenanceLog([]); // Clear logs on reset
  ...
  // Backend interaction functions
   const addLog = async (severity, message) => {
    try {
      ...
      setAlert({ message: "⚠️ Error communicating with server (addLog)" });
    }
  };
  ...
  const clearLogs = async () => {
    try {
      ...
      setAlert({ message: "⚠️ Error communicating with server (clearLogs)" });
    }
  };
  ...
  const faultProbData = [
    { name: 'Motor', value: Math.max(0, (100 - health) * 0.4) },
    { name: 'Joint', value: Math.max(0, (100 - health) * 0.3) },
    { name: 'Power', value: Math.max(0, (100 - health) * 0.2) },
    { name: 'Control', value: Math.max(0, (100 - health) * 0.1) }
  ];
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: "var(--color-bg)", minHeight: "100vh", padding: "2rem", color: "var(--color-text-primary)" }}>
      ...
      <style>{`
        ...
        body.dark-mode {
          ...
          --color-bg-secondary: #1f2937; /* Darker card background */
          --color-bg-tertiary: #374151; /* Dark hover */
          --color-text-primary: #f9fafb; /* Light text */
          --color-text-secondary: #d1d5db; /* Lighter secondary text */
        ...
        .button-primary {
           padding: 0.6rem 1rem; border-radius: 8px; border: none; background: var(--color-primary); color: white; font-weight: 600;
        }
        .button-primary:hover:not(:disabled) { background: var(--color-primary-hover); }
        .button-secondary {
           padding: 0.6rem 1rem; border-radius: 8px; border: 1px solid var(--color-border); background: var(--color-bg-secondary); color: var(--color-text-primary); font-weight: 500;
        }
        .button-secondary:hover:not(:disabled) { background: var(--color-bg-tertiary); }
         .button-danger {
           padding: 0.6rem 1rem; border-radius: 8px; border: none; background: var(--color-danger); color: white; font-weight: 600;
         }
         .button-success {
            padding: 0.6rem 1rem; border-radius: 8px; border: none; background: var(--color-success); color: white; font-weight: 600;
         }
         .button-icon {
            background: none; border: none; padding: 0.5rem; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: var(--color-text-secondary);
         }
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem", marginBottom: "1rem", flexWrap: "wrap", padding: "1rem", background: "var(--color-bg-secondary)", borderRadius: "16px", boxShadow: "var(--shadow-lg)", border: "1px solid var(--color-border)" }}>
        {!isShutdown ? (
          ...
          <div style={{ margin: "1rem", padding: "2rem", background: "#1f2937", borderRadius: "16px", border: "3px solid var(--color-danger)", textAlign: "center" }}>
            ...
            <div style={{ color: "var(--color-text-muted)", fontWeight: 500 }}>Health Depleted or Manually Stopped</div>
            <button onClick={restart} className="button-success" style={{ marginTop: "1rem", fontSize: "1rem", animation: "pulse 2s infinite" }}>
                  🔄 RESTART SYSTEM
            </button>
          </div>
        )}
        ...
        {!isShutdown && (
            <>
                <div style={{ background: "var(--color-bg-tertiary)", padding: "1rem", borderRadius: "12px", boxShadow: "var(--shadow-sm)", border:"1px solid var(--color-border)", textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: "1.2rem", fontWeight: 700, color: "var(--color-success)" }}>ACTIVE</div>
                ...
                <div style={{ background: "var(--color-bg-tertiary)", padding: "1rem", borderRadius: "12px", boxShadow: "var(--shadow-sm)", border:"1px solid var(--color-border)", textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: "1.2rem", fontWeight: 700, color: "var(--color-warning)" }}>{mttf}h</div>
                </div>
            </>
        )}
      </div>
       ...
       <div style={{ textAlign: "center", margin: "1rem 0" }}>
        {alert?.message && (
          <div style={{
            background: isShutdown ? "var(--color-danger-bg)" : alert.message.includes("✅") || alert.message.includes("Connecting") ? "var(--color-success-bg)" : alert.message.includes("Error") ? "var(--color-danger-bg)" : "var(--color-warning-bg)",
            color: isShutdown ? "var(--color-danger-text)" : alert.message.includes("✅") || alert.message.includes("Connecting") ? "var(--color-success-text)" : alert.message.includes("Error") ? "var(--color-danger-text)" : "var(--color-warning-text)",
            display: "inline-block", padding: "0.75rem 1.25rem", borderRadius: "10px", margin: "0.5rem 0",
            fontWeight: 500, border: isShutdown ? `2px solid var(--color-danger)` : `1px solid ${alert.message.includes("✅") || alert.message.includes("Connecting") ? 'var(--color-success)' : alert.message.includes("Error") ? 'var(--color-danger)' : 'var(--color-warning)'}`,
        ...
        {notificationSent && (
          <div style={{
            ...
            background: "var(--color-info-bg)", color: "var(--color-info-text)", padding: "0.5rem 1rem", borderRadius: "6px",
            margin: "0.5rem auto", maxWidth: "350px", fontSize: "0.9rem", fontWeight: 600, border: "1px solid var(--color-primary)"
          }}>
      ...
      {!isShutdown && (
         <>
            <div style={{
                display: "flex", justifyContent: "center", gap: "0", background: "var(--color-bg-secondary)",
                borderRadius: "12px 12px 0 0", marginTop: "2rem", boxShadow: "var(--shadow-md)", flexWrap: "wrap", borderBottom: "1px solid var(--color-border)"
            }}>
                    ...
                    <div style={{ marginBottom: "1.5rem", background:"var(--color-bg)", border: "1px solid var(--color-border)", borderRadius: "12px", padding: "1rem" }}>
                       <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem", cursor: "pointer" }}
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "8px"}}>
                            <Wrench size={20} color="var(--color-text-secondary)"/>
                            <h3 style={{ margin: 0, fontSize: "1.1rem", fontWeight: 600, color: "var(--color-text-primary)" }}>Fault Status Monitor</h3>
                        </div>
                        {expandedSections.faults ? <ChevronUp size={20} color="var(--color-text-secondary)"/> : <ChevronDown size={20} color="var(--color-text-secondary)"/>}
                      </div>
                      {expandedSections.faults && (
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))", gap: "1rem" }}>
                          {Object.entries({
                             ...
                             <div key={key} style={{
                              background: "var(--color-bg-secondary)", padding: "1rem", borderRadius: "8px", border: "1px solid var(--color-border)",
                              ...
                              <button onClick={() => toggleFault(key)} disabled={isShutdown} style={{
                                background: "none", border: "none", cursor: isShutdown ? "not-allowed" : "pointer", padding: 0
                              }}>
                                <FaultBadge status={faults[key] || 'OK'} />
                              </button>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="chart-card">
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                           <LineChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                             ...
                             <XAxis dataKey="time" tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} stroke="var(--color-border)"/>
                             <YAxis tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} stroke="var(--color-border)"/>
                             <Tooltip contentStyle={{ backgroundColor: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', color: 'var(--color-text-primary)' }}/>
                             <Line type="monotone" dataKey="motor_temp" stroke="var(--color-danger)" strokeWidth={2} dot={false} isAnimationActive={false}/>
                          </LineChart>
                        </ResponsiveContainer>
                      </div>
                      <div className="chart-card">
                      ...
                      <div className="chart-card">
                        ...
                        <ResponsiveContainer width="100%" height={200}>
                            <LineChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                             ...
                             <YAxis tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} stroke="var(--color-border)"/>
                             <Tooltip contentStyle={{ backgroundColor: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', color: 'var(--color-text-primary)' }}/>
                             <Line type="monotone" dataKey="rpm" stroke="#06b6d4" strokeWidth={2} dot={false} isAnimationActive={false}/>
                           </LineChart>
                        </ResponsiveContainer>
                       </div>
                    </div>
                       ...
                       <svg width="100%" height="350" viewBox="0 0 600 380" preserveAspectRatio="xMidYMid meet">
                        ...
                        <g transform="translate(300, 350)">
                          ...
                          <g style={{ transition: 'transform 0.3s ease-out' }} transform={`rotate(${(latest.j1_angle || 45) - 45})`}>
                            ...
                            <g style={{ transition: 'transform 0.3s ease-out' }} transform={`translate(0, -120) rotate(${(latest.j2_angle || 60) - 60})`}>
                               <rect x="-12" y="-100" width="24" height="100" fill={isDarkMode ? "url(#armGradientDark)" : "url(#armGradient)"} stroke={isDarkMode ? "#3b82f6" : "#1e40af"} strokeWidth="2" rx="6" />
                               <circle cx="0" cy="-100" r="15" fill="#f59e0b" stroke="#92400e" strokeWidth="2" />
                              <text x="0" y="-96" textAnchor="middle" fill="white" fontSize="9" fontWeight="bold">J2</text>
                              <g style={{ transition: 'transform 0.3s ease-out' }} transform={`translate(0, -100) rotate(${(latest.j3_angle || 30) - 30})`}>
                                <rect x="-10" y="-80" width="20" height="80" fill={isDarkMode ? "url(#armGradientDark)" : "url(#armGradient)"} stroke={isDarkMode ? "#3b82f6" : "#1e40af"} strokeWidth="2" rx="5" />
                                <circle cx="0" cy="-80" r="12" fill="#10b981" stroke="#065f46" strokeWidth="2" />
                                <text x="0" y="-76" textAnchor="middle" fill="white" fontSize="8" fontWeight="bold">J3</text>
                                <g style={{ transition: 'transform 0.3s ease-out' }} transform={`translate(0, -80) rotate(${(latest.j4_angle || 90) - 90})`}>
                                  <rect x="-8" y="-60" width="16" height="60" fill="#8b5cf6" stroke="#5b21b6" strokeWidth="2" rx="4" />
                                  <circle cx="0" cy="-60" r="10" fill="#a78bfa" stroke="#5b21b6" strokeWidth="2" />
                                  <text x="0" y="-56" textAnchor="middle" fill="white" fontSize="8" fontWeight="700">J4</text>
                                  <g style={{ transition: 'transform 0.3s ease-out' }} transform={`translate(0, -60) rotate(${(latest.j5_angle || 120) - 120})`}>
                                     <rect x="-7" y="-50" width="14" height="50" fill="#06b6d4" stroke="#0e7490" strokeWidth="2" rx="4" />
                                     <circle cx="0" cy="-50" r="9" fill="#22d3ee" stroke="#0e7490" />
                                    <text x="0" y="-46" textAnchor="middle" fill="white" fontSize="7" fontWeight="700">J5</text>
                                    <g style={{ transition: 'transform 0.3s ease-out' }} transform={`translate(0, -50) rotate(${(latest.j6_angle || 180) - 180})`}>
                       ...
                       <div className="chart-card">
                         ...
                         <ResponsiveContainer width="100%" height={180}>
                            <LineChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                             ...
                             <YAxis tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} stroke="var(--color-border)"/>
                             <Tooltip contentStyle={{ backgroundColor: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', color: 'var(--color-text-primary)' }}/>
                             <Line type="monotone" dataKey="j1_angle" stroke="#3b82f6" strokeWidth={2} dot={false} isAnimationActive={false}/>
                             <Line type="monotone" dataKey="j2_angle" stroke="#f59e0b" strokeWidth={2} dot={false} isAnimationActive={false}/>
                           </LineChart>
                         </ResponsiveContainer>
                       </div>
                       <div className="chart-card">
                         ...
                         <ResponsiveContainer width="100%" height={180}>
                            <LineChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                             ...
                             <YAxis tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} stroke="var(--color-border)"/>
                             <Tooltip contentStyle={{ backgroundColor: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', color: 'var(--color-text-primary)' }}/>
                             <Line type="monotone" dataKey="j3_angle" stroke="#10b981" strokeWidth={2} dot={false} isAnimationActive={false}/>
                             <Line type="monotone" dataKey="j4_angle" stroke="#8b5cf6" strokeWidth={2} dot={false} isAnimationActive={false}/>
                           </LineChart>
                         </ResponsiveContainer>
                       </div>
                        <div className="chart-card">
                         ...
                         <ResponsiveContainer width="100%" height={180}>
                           <LineChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                             ...
                             <YAxis tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} stroke="var(--color-border)"/>
                             <Tooltip contentStyle={{ backgroundColor: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', color: 'var(--color-text-primary)' }}/>
                             <Line type="monotone" dataKey="j5_angle" stroke="#ec4899" strokeWidth={2} dot={false} isAnimationActive={false}/>
                             <Line type="monotone" dataKey="j6_angle" stroke="#06b6d4" strokeWidth={2} dot={false} isAnimationActive={false}/>
                           </LineChart>
                         </ResponsiveContainer>
                       </div>
                    </div>
                       ...
                       <div className="chart-card">
                         ...
                         <ResponsiveContainer width="100%" height={240}>
                           <BarChart data={faultProbData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                             <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)"/>
                             <XAxis dataKey="name" tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} stroke="var(--color-border)"/>
                             <YAxis tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} stroke="var(--color-border)"/>
                             <Tooltip contentStyle={{ backgroundColor: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', color: 'var(--color-text-primary)' }}/>
                       ...
                       <div className="chart-card">
                         ...
                         <ResponsiveContainer width="100%" height={240}>
                            <LineChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                             <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)"/>
                             <XAxis dataKey="time" tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} stroke="var(--color-border)"/>
                             <YAxis domain={[0, 1]} tick={{ fontSize: 10, fill: 'var(--color-text-muted)' }} stroke="var(--color-border)"/>
                             <Tooltip contentStyle={{ backgroundColor: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', color: 'var(--color-text-primary)' }}/>
                             <Line type="monotone" dataKey="anomaly_score" stroke="var(--color-danger)" strokeWidth={2} dot={false} isAnimationActive={false}/>
                           </LineChart>
                         </ResponsiveContainer>
                       </div>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1.5rem" }}>
                         ...
                         <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                           <div style={{ fontWeight: 600, color: "var(--color-text-primary)" }}>Recent Maintenance Log</div>
                            <div style={{ display: "flex", gap: "0.5rem" }}>
                             <button onClick={clearLogs} title="Clear Logs" style={{ padding: "0.4rem 0.6rem", borderRadius: "6px", border: "1px solid var(--color-danger)", background: "var(--color-danger-bg)", color: "var(--color-danger-text)", fontWeight: 500 }}>Clear</button>
                             ...
                             <div style={{ color: 'var(--color-text-secondary)', textAlign: 'center', padding: '2rem 0' }}>No logs yet.</div>
                              ...
                              maintenanceLog.map(item => (
                                 ...
                                 <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem', alignItems: 'center' }}>
                                     <span style={{ fontWeight: 700, fontSize: '0.8rem', padding: '0.1rem 0.4rem', borderRadius: '4px', marginRight: '0.5rem',
                                         background: item.severity === 'CRITICAL' ? 'var(--color-danger-bg)' : item.severity === 'HIGH' ? 'var(--color-warning-bg)' : 'var(--color-bg-tertiary)',
                                         color: item.severity === 'CRITICAL' ? 'var(--color-danger-text)' : item.severity === 'HIGH' ? 'var(--color-warning-text)' : 'var(--color-text-secondary)'
                                     }}>
                                         {item.severity}
                                     </span>
                                     <span style={{ color: 'var(--color-text-muted)', fontSize: '0.75rem' }}>{item.timestamp}</span>
                                 </div>
                                 <div style={{ color: 'var(--color-text-primary)', wordBreak: 'break-word' }}>{item.message}</div>
                                 </div>
                             ))
                           )}
                         </div>
                       ...
                       <div style={{ background: "var(--color-bg)", padding: "1rem", borderRadius: "12px", border: "1px solid var(--color-border)", display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                         ...
                         <div style={{ marginTop: 'auto', paddingTop: '1rem', borderTop: `1px solid var(--color-border)` }}>
                           ...
                           <div style={{ display: 'flex', gap: '0.5rem' }}>
                             <button onClick={() => { if(isShutdown) return; sendNotif()}} disabled={isShutdown} className="button-primary" style={{ padding: '0.45rem 0.8rem', opacity: isShutdown ? 0.6 : 1 }}>Ping Team</button>
                             <button onClick={() => { if(isShutdown) return; addLog('INFO', 'Performed diagnostics'); }} disabled={isShutdown} className="button-secondary" style={{ padding: '0.45rem 0.8rem', opacity: isShutdown ? 0.6 : 1 }}>Diagnostics</button>
                           </div>
                         </div>
                       </div>
                    </div>
